        ...
        Utils.wait({
            ...
            until: function() {
                if (!self._options.carousel) {
					h = self.$( 'thumbnails-container' ).height();
					self.$( 'thumbnails-container' ).css({height: "0px"});
					h += self.$( 'stage' ).height();
					self.$( 'stage' ).css({height: h+"px"});
				}
        ...
        next.load( src, function( next ) {
            ...
            self._scaleImage( next, {
                ...
                complete: function( next ) {
                    ...
                    if ( data.layer ) {
						if (data.link) {
						  $(self._layers[ 1-self._controls.active ] ).css({ cursor: 'pointer' }).bind( 'mouseup', function() {		
                              // popup link
                              if ( data.link ) {
                                  if ( self._options.popupLinks ) {
                                      win = window.open( data.link, '_blank' );
